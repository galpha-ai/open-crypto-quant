...
where
    T: ?Sized + Serialize,
{
    // Decode the secret from URL-safe base64 (Python uses urlsafe_b64decode)
    let decoded_secret = base64::engine::general_purpose::URL_SAFE
        .decode(secret)
        .map_err(|e| PolyfillError::crypto(format!("Invalid base64 secret: {}", e)))?;
    ...
    // Encode with URL-safe base64 (Python uses urlsafe_b64encode)
