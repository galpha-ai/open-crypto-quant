...
use crate::http_config::{
    ...
    create_optimized_client_with_proxy, prewarm_connections,
};
...
impl ClobClient {
    ...
    /// Create a client with L2 headers and proxy wallet configuration
    ///
    /// Use this for email/Magic wallets or browser wallet proxies:
    /// - `signature_type`: 1 for PolyProxy, 2 for PolyGnosisSafe
    /// - `funder`: The proxy wallet address that holds your funds
    pub fn with_proxy_wallet(
        host: &str,
        private_key: &str,
        chain_id: u64,
        api_creds: ApiCreds,
        ...
        let sig_type = match signature_type {
            1 => crate::orders::SigType::PolyProxy,
            2 => crate::orders::SigType::PolyGnosisSafe,
            _ => crate::orders::SigType::Eoa,
        };

        let funder_address = funder
            .parse::<Address>()
    ...
    /// Create a client with L2 headers, proxy wallet configuration, and HTTP proxy
    ///
    /// Use this for email/Magic wallets or browser wallet proxies with HTTP proxy:
    /// - `signature_type`: 1 for PolyProxy, 2 for PolyGnosisSafe
    /// - `funder`: The proxy wallet address that holds your funds
    /// - `http_proxy_url`: HTTP proxy URL in format "http://user:pass@host:port"
    pub fn with_proxy_wallet_and_http_proxy(
        host: &str,
        private_key: &str,
        chain_id: u64,
        api_creds: ApiCreds,
        ...
        let http_client = create_optimized_client_with_proxy(http_proxy_url)
            .unwrap_or_else(|_| Client::new());
    ...
    /// Update the cached balance and allowance for specific tokens.
    /// This is a trigger/notification method that doesn't return data.
    pub async fn update_balance_allowance(
        ...
        // Check status code - this API returns empty response on success
        if response.status().is_success() {
            Ok(())
        } else {
            let status = response.status().as_u16();
            let body = response.text().await.unwrap_or_default();
            Err(PolyfillError::api(status, body))
        }
    }
